...
{
    if (fbertit[cpu].find(tag) == fbertit[cpu].end()) return;

    fberti_t *tmp = fbertit[cpu][tag];
...
void fberti_table_add(uint64_t tag, uint32_t cpu, int64_t stride)
{
     ...
     *  - stride: actual cpu
     */
    if (fbertit[cpu].find(tag) == fbertit[cpu].end())
    {
        // FIFO MAP
        if (fbertit_queue[cpu].size() > BERTI_TABLE_SIZE)
        {
            uint64_t key = fbertit_queue[cpu].front();
            fberti_t *tmp = fbertit[cpu][key];
            delete tmp->stride;
            delete tmp;
            fbertit[cpu].erase(fbertit_queue[cpu].front());
            fbertit_queue[cpu].pop();
        }
        fbertit_queue[cpu].push(tag);

        assert(fbertit[cpu].size() <= BERTI_TABLE_SIZE);

        fberti_t *tmp = new fberti_t;
        ...
        // Save value
        fbertit[cpu].insert(make_pair(tag, tmp));
        return;
    }
...
uint8_t fberti_table_get(uint64_t tag, uint32_t cpu, stride_t res[MAX_PF])
{
     ...
     */
    if (!fbertit[cpu].count(tag)) return 0;
...
{ 
    ...
    for (uint32_t i = 0; i < num_on_time; i++)
    {
        ...
        {
            // Only useful strides
            fberti_table_add(ip[i], cpu, stride);
        }
    }
}
        ...
        uint8_t type, uint8_t critical_ip_flag)
{
    ...
    if (!fberti_table_get(ip, cpu, stride)) return;
